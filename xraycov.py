...
full = pd.read_csv(r'C:\Users\mugdhapolimera\github\SDSS_Spectra\ECO+RESOLVE_filter_new.csv') #galaxies with BPT SNR > 2
...
'''
xmm3_m1 = glob.glob('xray_imgs\m1\*\pps\*8000*.FTZ')
...
def getxraycov_byimg(imglist, racomp, deccomp):
    ...
    print ('matchedgals', matchedgals)
    print ('unmatchedgals', unmatchedgals)
    return matchedgals, np.array(matchedfields), unmatchedgals,\
             np.array(ra_centarr), np.array(dec_centarr)
class Mosaic:
    '''
...
#    np.savetxt('catalogs/xraycov/deccent_' +fname+'_xrcovg_fields_set.txt', deccent)
    ...
    return matched_gals, matched_fields,unmatched_gals,racent, deccent
...
def plotcov( matched_fields, save=False,fname=''):
    ...
    plt.tight_layout()
    if save:
        fig.savefig('covgmap_xmm_chand_GSW'+fname+'.png',dpi=250)
        fig.savefig('covgmap_xmm_chand_GSW'+fname+'.pdf',dpi=250,format='pdf')
...
def plotstuff(im, save=False):
    ...
    if save:
        ...
        fig.savefig('field.pdf',dpi=250, format='pdf')
        fig.set_rasterized(True)
        fig.savefig('field.eps',format='eps')
        plt.close(fig)
    fig = plt.figure()
    ax = fig.add_subplot(111,projection=im.wcs)
    ...
    plt.tight_layout()
    if save:
        fig.savefig('field_dilat.png',dpi=250)
        fig.savefig('field_dilat.pdf',dpi=250, format='pdf')
        fig.set_rasterized(True)
        fig.savefig('field_dilat.eps',format='eps')
        plt.close(fig)
    fig = plt.figure()
    ...
    plt.tight_layout()
plotstuff(Mosaic(allimglists[24]))
plotcov(matched_fields, save=False)
